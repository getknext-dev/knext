 ...
 *  - run 28558576615: e2e-deploy.sh packed @getknext/core with `npm pack`, which
 *    ships pnpm's raw `workspace:^` dep on @getknext/lib verbatim. Every fixture's
 ...
 *  - runs 29182334221/29184529993 (#255/#256): @getknext/core gained a
 *    `@getknext/db: workspace:^` dep (ADR-0021; `kn-next db migrate` dynamically
 *    imports `@getknext/db/migrate`), `pnpm pack` rewrote it to `^0.1.0`, but the
 *    workflow only packed lib+core — the preflight npm install 404'd on the
 *    unpublished @getknext/db in seconds. The gate worked; the pack set (and this
 *    script's hardcoded pair) did not.
 *
 * THE GATE (now dependency-graph-derived, no hardcoded package list):
 *   1. derive the REQUIRED tarball set from the @getknext/* dependency closure of
 *      @getknext/core, walking the PACKED tarball manifests (dependencies +
 ...
 *      guards dependency confusion: nothing is published under @getknext on
 *      npmjs yet (#53), so a future 4th workspace dep whose tarball is missing
 *      must never be silently satisfiable by whatever the registry returns.
 ...
 *      @getknext/* package must have resolved from a LOCAL tarball (file:),
 *      never a registry URL, and must belong to the derived closure;
 *   4. resolve smokes: `@getknext/core/adapter` must resolve to real JS (the
 *      exact subpath e2e-deploy.sh resolves for NEXT_ADAPTER_PATH), and
 *      `@getknext/db/migrate` must dynamically IMPORT — the exact import
 ...
 *   node scripts/e2e-preflight.mjs --tarballs-dir <dir with getknext-lib-*.tgz + getknext-db-*.tgz + getknext-core-*.tgz>
 */
...
const ROOT_PACKAGE = '@getknext/core';
const ADAPTER_SUBPATH = '@getknext/core/adapter';
const DB_MIGRATE_SUBPATH = '@getknext/db/migrate';
...
/** Newest tarball matching `<prefix>-*.tgz` in dir (pnpm/npm naming for @getknext/*). */
...
/**
 * BFS the @getknext/* dependency closure of `rootName`, reading each member's
...
function resolveClosure(dir, rootName) {
  ...
  while (queue.length > 0) {
    ...
    if (!tgz) {
      fail(
        `no ${prefix}-*.tgz found in ${dir}, but ${name} is in ${ROOT_PACKAGE}'s @getknext/* ` +
          `dependency closure — pack it with pnpm pack alongside the others (the @getknext scope ` +
          `is unpublished, #53: npm cannot and MUST NOT satisfy it from the registry). ` +
          `The #255/#256 incident was exactly this hole for @getknext/db.`,
      );
    }
    let manifest;
    try {
...
function main() {
  ...
  // 1. Derive the required tarball set from @getknext/core's dependency closure.
  const closure = resolveClosure(dir, ROOT_PACKAGE);
  const tarballs = closure.names.map((name) => closure.tarballByName[name]);
  console.log(
    `[e2e-preflight] @getknext/* closure of ${ROOT_PACKAGE}: ${closure.names.join(', ')}\n` +
  ...
  //    installed @getknext/* package is auditable (local tarball vs registry).
  ...
  // 3b. Lockfile audit: every @getknext/* package must have come from a LOCAL
  ...
  console.log(
    '[e2e-preflight] origin audit OK: every @getknext/* package came from a local tarball',
  );
  ...
  // `const { runMigrations } = await import("@getknext/db/migrate")`). A resolve-only
