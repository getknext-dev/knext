 ...
 * FUTURE @getknext/* workspace dep added to @getknext/core never trips a
 * workspace:-spec check — the packed manifest looks perfectly publishable. And
 * because nothing is published under @getknext on npmjs yet (#53), the scratch install
 * only fails loudly while that name is UNPUBLISHED; a foreign @getknext/x would
 ...
 *      @getknext/core (walking the packed manifests) — a member without a local
 *      tarball fails BEFORE any npm install runs;
 *   2. after the scratch install, assert every @getknext/* entry in the lockfile
 *      resolved from a LOCAL tarball (file:), never a registry URL.
 */

const KNEXT_SCOPE = '@getknext/';

/** `@getknext/db` → `getknext-db` (the `pnpm pack` tarball filename prefix). */
...
/**
 * The @getknext/* dependency names one INSTALL of `manifest` pulls in:
...
/**
 * Verify every @getknext/* package in a scratch-install lockfile (npm
 ...
 * @param {Set<string>} allowedNames the derived @getknext/* closure (including the root)
 */
export function assertLocalKnextResolutions(lockfile, allowedNames) {
  ...
  for (const [path, entry] of Object.entries(packages)) {
    ...
    // (nested paths like `node_modules/@getknext/lib/node_modules/pino` are pino,
    // not an @getknext package).
    const name = path.split('node_modules/').pop() ?? '';
    if (!name.startsWith(KNEXT_SCOPE)) continue;
    if (!allowedNames.has(name)) {
      problems.push(
        `${name}@${entry?.version ?? '?'} appeared in the scratch install but is NOT in the ` +
          `@getknext/* dependency closure derived from the packed tarballs — an unexpected ` +
    ...
    if (typeof resolved === 'string' && /^https?:\/\//i.test(resolved)) {
      problems.push(
        `${name}@${entry?.version ?? '?'} resolved from a REGISTRY URL (${resolved}) instead of ` +
          `the local tarball — nothing is published under @getknext on npmjs yet (#53), so this is ` +
