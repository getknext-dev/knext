...
// `next-env.d.ts` is GENERATED by Next on every dev/build run (and gitignored).
// Next itself writes `import './.next/dev/types/routes.d.ts'` into it, so the
// no-TS-extension rule cannot apply — it is not authored source. Scoped to that
// exact basename so hand-written declaration files stay covered.
...
function collectFiles(target) {
  ...
  for (const entry of readdirSync(target, { withFileTypes: true })) {
    ...
    } else if (entry.isFile() && SOURCE_EXT.test(entry.name) && !SKIP_FILES.has(entry.name)) {
