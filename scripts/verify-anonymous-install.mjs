...
export function auditAnonymousWorkflowJob(workflowText) {
  ...
  // fired only when the block ALSO contained `uses:`, so a flow-style step
  // carrying just a `run:` + a credential `env:` slipped through with zero
  // findings. Widened: if `steps:` is present at all but no `- ` item parsed,
  // that is the finding — the audit's job is to be fail-closed, and a shape it
  // cannot inspect is a violation, not a pass. The real anonymous-install
  ...
  if (steps.length === 0 && hasStepsKey) {
    findings.push(
      `job \`${jobId}\` declares \`steps:\` but no \`- \` block-sequence item could be parsed ` +
        'from it (a flow-style `steps: [ … ]` value, a mapping, or otherwise unparseable) — the ' +
        'per-step credential rules cannot inspect it, so it fails closed rather than passing vacuously',
    );
  }
