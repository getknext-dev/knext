 ...
 * $PORT. The extra routes emit the EXACT origin Cache-Control values Next's
 * standalone server produces (getCacheControlHeader + the pages-router
 * fallback shell), so the contract test can assert the deploy script's
 * serving layer normalizes them to the deployed-platform values the official
 * deploy suite expects (#175, prerender.test.ts caching-header failures).
 */
