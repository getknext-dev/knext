...
| `PoolMax` | `poolMax` | enforces `maxScale × poolMax ≤ 80` (the app connection budget, ADR-0028) at admission **and** injects `KNEXT_DB_POOL_MAX` into the app container so `@getknext/lib`'s `getDbPool()` caps the pg pool `max` at runtime (ADR-0029, #378) | *unset (no check, no env)* |
