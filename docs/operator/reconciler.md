...
### 2. Asynchronous ISR Binding (`KafkaSource`)
...
### 3. Application Server (`serving.knative.dev/v1 Service`)
...
- **Volumes**: None. The V8 compile cache is baked into the image at build time (ADR-0035), so no PVC is provisioned or mounted.
