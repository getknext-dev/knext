...
spec:
  scaling:
    ...
    poolMax: 5                # Optional per-pod DB pool max; when set the operator enforces maxScale × poolMax ≤ 100 (ADR-0028)
```
...
> the operator enforce the connection-wall invariant `maxScale × poolMax ≤ 100`.
