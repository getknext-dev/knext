...
(31 runs: 27 node nights carrying a fingerprint, 1 pre-fingerprint node night, 4 bun weeklies),
graded by `scripts/compat-window-audit.mjs` — which exists because this reconstruction had now been
done by hand twice, and #545 AC 3 asks for exactly that number to stop being folklore.
...
defect" needs its converse said too:** the shard set has *not* moved since. Three consecutive
weeklies are byte-identical — `775 passed / 3 failed`, shards 6 and 8, the same three files. The bun
lane is **deterministically red**, and #545's re-run-until-green vector cannot operate on it.
...
14** — not because a night went red, but because the harness fingerprint moved **9 times in 27
nights**.
...
The frozen set includes the packed `@getknext/*` closure, and this repo merges to `main` most days;
the 7-night stretch corresponds exactly to the quietest week for shipped `dist/**` bytes. **The v1.0
compat gate is a scheduling problem — it needs a ~2-week freeze on the packed closure — not a
defect-fixing one.** That is a design consequence of ADR-0039's freeze scope, and whoever owns the
v1.0 date should plan the freeze rather than wait for a streak that the merge cadence forbids.
...
- **High** on the bun lane's determinism in shard-and-file: three byte-identical consecutive runs.
...
*Evidence for §8: `compat-run-ledger` artifacts from all 31 scheduled runs 2026-07-28 → 2026-08-24
