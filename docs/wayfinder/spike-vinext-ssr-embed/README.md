...
| `e2-bisect.sh` | Bisects which vinext version emits a runtime `require("react-dom")` in the SSR chunk. **Vite-7-only** (pins vite `^7`, plugin-react `^5`) — it cannot produce a 7-vs-8 comparison; use `e2-vite78-fixed.sh` for that. |
