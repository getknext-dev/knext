...
| **I**nfo disclosure | Vulnerable deps shipped silently. | **syft SBOM** per image; **Trivy fails the build on HIGH/CRITICAL** on main (`--ignore-unfixed`) — and the app image is **scanned BEFORE it is pushed** (`supply-chain.yml`: build local → SBOM → Trivy gate → push → sign; guarded by `tests/supply-chain-workflow.test.ts`), so a scan-failed image is never pullable at a stable tag and never signed. Builder pinned to a patched Go release (`check-trivy-baseimage.bats.sh`). | Apply the same scan-before-push ordering to `operator-supply-chain.yml` (it still pushes at build time). |
| **R**epudiate | Can't prove which source built an image. | buildkit **provenance** attestation on main for the operator image; **cosign SBOM attestation + keyless signature** for both images. | The app image's buildkit provenance manifest was dropped when the flow moved to scan-before-push (the local `docker` exporter can't carry a provenance manifest list); restore it via an OCI-layout build + `crane` push if provenance is required for the app image too. |
