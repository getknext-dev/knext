...
| --- | --- | --- | --- |
| `0.4.2` (in tree) | `apps.kn-next.dev/v1alpha1` | `operator-latest` | patch. Fixes the scaffolded-app `npm install` crash on npm 10.x: the app template pinned `vitest@^4`, whose Vite peer range excludes the `vite@8` the template also pins, so npm 10 arborist aborts (npm 11 resolved it, hiding the bug); moved to `vitest@^5`. Also adds structured repair hints to `doctor` failure output. **Non-breaking**; same CRD (`v1alpha1`) and operator bundle as `0.4.1`. Upgrade operator/CRD first, then CLI. |
