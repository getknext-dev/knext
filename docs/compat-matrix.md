...
A `bun-version` `workflow_dispatch` input (string, default **pinned** — `'1.3.14'`, matching the CI pin, #754; dispatch-only — the weekly schedule runs the pinned fallback, and dispatch materialises the input default, so the pin holds on every path unless a version is passed explicitly)
