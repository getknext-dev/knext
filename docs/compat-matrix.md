...
| Graceful shutdown (SIGTERM drain) | ✅ | examples/bun-exec/test/alpine-image.docker-e2e.test.ts, packages/kn-next/src/adapters/shutdown.ts | E2E-proven on the SHIPPED artifact: the alpine image e2e delivers a real `SIGTERM` to the compiled binary in a clean container and asserts the in-flight request completes, exit 0, and the drain markers in order (mutation-proved — a force-stop mutant drops the request and reds exactly this test). The unit-tested standalone-runtime drain (`shutdown.ts`) remains as the LEGACY shape's evidence. |
