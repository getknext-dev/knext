...
| 5 | 2026-08-21 (`/dashboard`, n=8, canonical in-cluster instrument; bench pod `bench-timer2` — NOTE: **same node as fm-node** (10.0.1.78), unlike prior sittings, which is the likely cause of warm 125.5→45ms and exec-gap 3866→1763ms; same-pod deltas remain clock-safe) | Since row 4: activator reset to pod-direct (row 4's finding); #812/#814 operator probe fix live (affects operator-deployed apps, NOT this hand-applied benchmark); merged plane otherwise unchanged | **4118** | 161 | **45** | **3/8 at 7.5–8.1s** (7452/7611/8063ms lazy, ALL success bodies) | **Wake median moved for the first time since row 0: 4917.5→4118 (−16%), consistent with the activator pod-direct path** (the Ready→routable term collapsed in the row-4 phase measurements; this is the canonical instrument confirming it end-to-end). Lazy floor flat (117.5 vs 112). **The multi-second tail REGRESSED: 1/8 @2.4s → 3/8 @7.5–8.1s, row-1 magnitude, but success bodies (no fallbacks) and a different plane era** — mechanism still undiscriminated (the three readings of row 3 stand). The [slow-dep] discriminator (#803) is MERGED but the deployed image (digest b7b7abc…) predates it — the next sitting rolls the instrumented image and reads the `[slow-dep]` lines on the stall cycles. Medians verbatim from the instrument summary; zero retries |
| 6 | 2026-08-21 (`/dashboard`, n=8, canonical instrument + per-cycle `[slow-dep]` capture; revision `fm-node-00095`, image built from main @ digest 635a0067…, amd64 — the first sitting with the #803 discriminator DEPLOYED) | fm image rebuilt from merged main (docker build --platform amd64 → docker save → **crane push with the cluster's own ocir-pull creds** — no local keychain; an arm64 first attempt exec-format-erred and was left unrouted), rolled env-safely (full-template apply, REDIS_URL asserted) | 4217.5 | — (see mechanism) | 46 | **6/8 at 7.7–20.1s, most `outcome:error`, incl. warm-request errors** | **ROW 3'S THREE-WAY QUESTION IS ANSWERED: every one of 9 captured `[slow-dep]` lines is `dep:pg, op:pool.query` — zero redis lines across all stalls. The eager-cache-connect lever is DEAD; the tail belongs to the PG path.** The sitting ALSO caught a live plane degradation: the app logged `getaddrinfo EAI_AGAIN pggw-apps.scale-zero-pg.svc.cluster.local.` — **EAI_AGAIN on the ROOTED name**, so rooting (#796/#800) removed the search-walk multiplier but the single lookup still dies when the resolver path does. CoreDNS itself: logs clean, 2–3m CPU — but **both replicas are colocated on 10.0.1.78, the 99%-CPU-allocated node that also runs fm-node and the bench pod**; the failure shape (UDP to a healthy resolver on a saturated node, cache-handler ETIMEDOUT riding the same wave) is the conntrack/node-contention class. Durations quantize at ~10.1s and ~15.0s (the pool-connect timeout), fallback bodies on error cycles. **Tail magnitude this row is confounded by the live degradation — the DISCRIMINATION verdict is not** (it is the app's own in-process instrument). Levers, now evidence-backed and both infra-owned: spread CoreDNS off the hot node (podAntiAffinity) and/or NodeLocal DNSCache; plane desaturation. The in-repo lever burden shifts to the DB path: the writer pool's first-query cost and the 15s connect timeout's interaction with gateway wakes (#779/#781 territory) |

| 7 | 2026-08-24 (`/dashboard`, canonical in-cluster instrument, bench-timer3; **A/B on the CPU request, #593** — the lever PR #843's decomposition named after retiring #441's premise) | Arm A = fm-node as it had always run: **no resource requests at all** (the 0-CPU-request condition the decomposition fingered). Arm B = `requests cpu 100m/memory 256Mi` (rev `fm-node-00097`), same image, same day, full-template apply with REDIS_URL asserted | A **3875** / B **4391.5** | — | — | both arms: PG-path lazy stalls on nearly every cycle (medians A 10101 / B 10129.5) | **NULL-TO-NEGATIVE, and the constraint is the finding.** A 100m request did not help — it measured **+516ms (+13.3%)** slower. Two facts make this more useful than the number alone: (1) **a 500m request could not be scheduled at all** — `0/2 nodes are available: 2 Insufficient cpu` — so the request size the contention hypothesis actually implies **exceeds what this plane can grant**; (2) on a saturated plane a request is a scheduling *constraint*, so asking for 100m plausibly costs more at schedule time than it buys in CPU share. **#593's question cannot be settled on this cluster while it is this saturated** — that is the honest answer, and it converts plane desaturation (user-owned) from a nice-to-have into a prerequisite for the entire contention line of work. fm-node was reverted to no-requests (its pre-experiment state) after the sitting |

### Row 7 per-arm data

| arm | n | wakes (ms) | wake median |
|---|---|---|---|
| A — no request | 7 valid (1 excluded: image pull, 14285ms) | 3252, 3526, 3526, 3875, 4250, 4618, 4773 | **3875** |
| B — cpu 100m | 8 | 3885, 3907, 4136, 4383, 4400, 4430, 4646, 4899 | **4391.5** |

**Instrument notes, so the next sitting does not re-pay them:** arm B initially recorded ZERO
cycles because revision `fm-node-00096` — the unschedulable 500m attempt — left a **Pending pod
forever**, and the harness waits for pods to reach zero before each cold cycle; scaling that
superseded deployment to 0 released it immediately. Arm B's harness then died during its 8th
cycle after recording 8 valid ones (the parent shell was reaped), which is why arm B has 8 and
arm A 7 — both are above the row's usual n, and the comparison stands. **Confounds stated:** the
arms are sequential (plane drift possible), and the PG-path tail was degrading throughout, hitting
nearly every cycle in BOTH arms (worse than row 6's 6/8) — it inflates `lazy` in both arms equally
