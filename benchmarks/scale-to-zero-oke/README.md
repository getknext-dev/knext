...
| `api retries: 0 (no transient API errors — clean control-plane run)` | Printed when the control plane answered every call first try, every scale-down wait observed a pod count, **and** pod queries were able to separate kubectl's stderr from its stdout. All three, or this line does not appear. |
