 ...
 * edge-scoped `IgnorePlugin` the knext adapter injects from `modifyConfig`
 * (#356/ADR-0031) is what actually excludes its Node-only subtree from the edge
 * bundle (#342/#344). The knext runtime runs the app on Node (the standalone
 * server), so no behavior is lost.
 *
 * This module MUST be loaded lazily (never statically imported from an
 * edge-compiled file) AND the adapter-injected edge IgnorePlugin MUST stay in
 * place — the guard in `apps/file-manager/instrumentation-edge-safe.test.ts`
