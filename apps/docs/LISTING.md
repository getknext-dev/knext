...
> `@getknext/core` is published to npm (`npx knext` must work for an outside user, or the listing
...
- [ ] **npm publish:** `@getknext/core` is published so `npx knext` works for outside users.
