...
knext official adapter (`@getknext/core/adapter`) and can deploy as a scale-to-zero Knative Service via a
...
- **`@getknext/core`** — the knext adapter + config types, consumed via `workspace:*`
