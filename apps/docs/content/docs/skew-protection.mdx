...
generated id, so building outside `knext deploy` is unchanged. Use `||` rather than `??` — with
...
<Callout type="warn">
If the deploy tag's asset prefix is not there after the build, `knext deploy` **aborts before
...
Local `next dev` and `next build` are unaffected — outside `knext deploy` the variable is unset
...
has already shipped) and is also available as a standalone command, `knext gc`, for scheduled or
manual runs — see the [CLI reference](/docs/cli#knext-gc).
...
Every storage-less deploy announces this mode explicitly, and `knext gc` reports there is
