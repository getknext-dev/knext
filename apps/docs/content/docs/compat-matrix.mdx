...
| Official Next.js compatibility suite (Node runtime) | ✅ | **Scope, stated plainly: this credential was earned on the Node standalone target, which new deployments can no longer select — the default build is the [compiled executable](/docs/bun-runtime), and this ✅ does not extend to it (see the row below).** Green: 778 passed / 0 failed (16 shards, Next.js v16.2.0, deploy mode, Node runtime), re-verified nightly. Covers Next's deploy-eligible e2e set minus 4 documented architectural exclusions (the ⛔ rows) and a small evidence-backed flaky-quarantine ledger — see [Verified compatibility](/docs/compat-suite). A red nightly withdraws this row. |
...
| Official suite — compiled executable (the shipped artifact) | ❌ | **No suite run has exercised the compiled executable yet** — its verification lane is being stood up. Until it publishes a green run, the shipped artifact's compatibility is **measured per feature** (the capability rows above), not suite-verified. The first published number may be low; we publish it anyway. |
