...
on the vinext single-executable target, where the runtime is compiled into the binary. On the
standalone targets (`turbopack` and `webpack`, identically) it decides how the server is packaged:
