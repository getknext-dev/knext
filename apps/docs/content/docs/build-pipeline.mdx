...
knext offers **two** build targets, and you choose between them with the `build` field.
...
This target has a prerequisite in your project, not just in the config: `kn-next build` runs your
app's own build script, so that script must run `next build` with `output: 'standalone'` in
`next.config.ts` — otherwise there is no standalone server for the image to copy. Apps created by
`kn-next create` are vinext apps whose build script runs `vite build` and whose `next.config.ts`
deliberately omits `output`, so switching such an app to this target means changing both first.

If you select `build: 'turbopack'` and your build script still doesn't produce
`.next/standalone` (for example, the build script wasn't updated), `kn-next build` fails
...
`kn-next` stages and builds the matching standalone runtime image for you; `runtime` has no effect
on the vinext single-executable target, where the runtime is compiled into the binary. On this
target it decides how the server is packaged:
