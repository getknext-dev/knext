...
- **Bytecode caching is the V8 compile cache, baked into the image.** The image
  ...
  `docker build`, warms `/api/health`, and ships the compiled code as a layer. Every pod starts
  with it, including the first one after scaling to zero. The image build **fails** if the warm-up
  request does not succeed or the cache comes out too small, rather than shipping an empty cache.
  To see the cache being used, start a container with `NODE_DEBUG_NATIVE=COMPILE_CACHE`: the log
  says the cache for `/app/.output/server/index.mjs` **was accepted**.
...
Then copy `knext-node-entry.mjs` from a freshly created app into yours.
