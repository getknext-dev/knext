...
on the vinext single-executable target, where the runtime is compiled into the binary. Today the
standalone target runs uncompiled (the supervisor spawns `server.js` directly); packaging it as a
single bytecode executable is planned but not yet available.
