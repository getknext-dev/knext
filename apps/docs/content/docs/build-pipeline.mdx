...
- **Bytecode caching is the V8 compile cache, baked into the image.** The image
  ...
  `kn-next.config.ts`, default `/api/health`) before shipping the compiled code as a layer. `kn-next
  build`/`deploy`/`preview` thread a custom `healthCheckPath` through automatically, so a custom
  health route works with nothing extra to configure. Every pod starts with the cache, including the
  first one after scaling to zero. The image build **fails** if the warm-up request does not succeed
  or the cache comes out too small, rather than shipping an empty cache — see
