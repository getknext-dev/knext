...
```bash title="shell"
knext init-ci --namespace my-app
```
...
`kubectl get ksvc -n <namespace>`, or `knext status`. An output that was empty on every dry run
...
By default the action runs the read-only `knext doctor` preflight and fails the job if the cluster
