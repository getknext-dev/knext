...
npx kn-next deploy -r registry.example.com/storefront          # build → push → apply
npx kn-next status storefront --watch                          # poll until Ready=True
npx kn-next rollback storefront --to storefront-00002          # pin 100% to a prior revision
npx kn-next db bind storefront --secret storefront-db          # DATABASE_URL from a Secret
