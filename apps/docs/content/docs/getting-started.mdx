...
**vinext**, an open-source Vite-based implementation of Next.js, compiled into a single fast executable. (The official Deployment Adapter API remains supported as the suite-verified legacy lane.)
...
{/* REMOVE the Callout below — and restore §0's unconditional "a plain npm i is all it takes"
    promise — once the `@getknext/*` packages are published to the public npm registry. */}
<Callout type="warn">
**Not on npm yet — a release gap on our side.** The `@getknext/*` packages are still being
published to the public registry. Until that lands, `npm i @getknext/core` fails with a
not-found error — that is our gap, **not a mistake in your setup**. To try knext before then,
install it from a checkout of the source repository; otherwise check back shortly.
</Callout>
