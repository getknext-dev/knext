...
(networking), cert-manager, and the knext operator. This guide only covers the GKE-specific pieces: cluster, storage, cache, and
registry.
</Callout>
...
GKE ships a default `standard-rwo` StorageClass, so stateful add-ons you run alongside knext (Postgres, Redis)
