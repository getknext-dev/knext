...
`DATABASE_URL` case below), so a new or updated CR can never carry one. A CR that already had the
collision before this rule existed keeps reconciling (grandfathered): for `HOSTNAME` the
platform's own value always wins (request routing depends on it), but for every other name your
`envMap` value wins instead, preserving what the app was already receiving. Either way it is
surfaced by a Warning event and the `EnvMapCollision` condition — remove the `envMap` entry to
resolve it.
