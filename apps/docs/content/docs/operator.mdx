...
override is surfaced by a Warning event and the `EnvMapCollision` condition; for `HOSTNAME` the
platform's own value always wins instead (request routing depends on it). Remove the `envMap`
entry to fall back to the platform's default.
...
than putting the URL in `spec.cache.url` / `spec.revalidation.kafkaBrokerUrl` directly.
