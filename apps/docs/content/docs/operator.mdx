...
| `cache` | `provider`, `url`, `keyPrefix` configure the **data cache** (ISR/data). `enableBytecodeCache`, `bytecodeCacheSize` are **deprecated** — the boot compile cache is now baked into the image by default, so you no longer need to enable them; they still work but will be removed in a future release. See [bytecode caching](/docs/bytecode). |
...
<Callout type="warn">
**Honest scope — `provisionKafkaSource: true` is rejected.** The `KafkaSource` this field would
provision sinks into an `<app>-revalidator` consumer that knext **does not build**, so enabling it
could only produce a source pointing at a Service that never comes up, with revalidation silently
inert. The operator therefore rejects `provisionKafkaSource: true` at admission with a
`not implemented` error until that consumer ships; the field stays in the API so enabling it later
is not a breaking change.

`queue: kafka` on its own is accepted and provisions nothing: the operator records a non-fatal
`RevalidationDeferred` status condition (`Ready` stays `True`) so the gap is visible in `status`
