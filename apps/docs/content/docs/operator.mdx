...
spec:
  ...
  cache:
    enableBytecodeCache: true                  # faster boot; the volume binds to ONE node
                                               # — extra pods on another node stay Pending
  storage:
    provider: gcs
...
| `cache` | `provider`, `url`, `keyPrefix` configure the **data cache** (ISR/data). `enableBytecodeCache`, `bytecodeCacheSize` configure the **boot cache** and are independent of the provider — either works without the other. `enableBytecodeCache` is off by default: its volume attaches to one node at a time, so combining it with a `maxScale` that spreads pods across nodes leaves those pods `Pending`. See [bytecode caching](/docs/bytecode). |
