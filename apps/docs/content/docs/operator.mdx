...
| `build` | `vinext` (the compiled single executable — the default the CLI writes), `turbopack`, or `webpack` (both the standalone shape — `next build` with different bundlers; `turbopack` is also what absence means). Drives how the operator starts the container. See [Bun & the compiled executable](/docs/bun-runtime). |
| `runtime` | `bun` or `node` (default `node`). Standalone shape only — for `build: "vinext"` the runtime ships inside the binary and this field has no effect on startup. |
