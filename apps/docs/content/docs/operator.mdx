...
| `build` | `vinext` (the compiled single executable — the default the CLI writes) or `turbopack` (the legacy standalone shape, also what absence means). Drives how the operator starts the container. See [Bun & the compiled executable](/docs/bun-runtime). |
