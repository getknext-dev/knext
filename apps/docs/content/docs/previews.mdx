...
A preview is a complete, ephemeral deployment of your app for a single pull request. `knext
preview deploy` builds a fresh image and applies a **new** `NextApp` CR named `<app>-pr-<n>` — its
own image, its own URL, its own assets, its own cache keyspace. `knext preview destroy` deletes
...
knext preview deploy --pr 42 --branch feature/cart

# Tear it down
knext preview destroy --pr 42
```
...
<Callout type="info">
The shippable mechanism — the `knext preview` CLI and the operator's preview reconcile — is fully
