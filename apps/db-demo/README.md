# db-demo — the `@getknext/db` example app

A minimal, runnable proof of the [knext data SDK](../../packages/db) (`@getknext/db`,
...
|---|---|---|
| Schema (1 table) | `src/db/schema.ts` | `@getknext/db/schema` (`pgTable`, columns) |
| drizzle-kit config | `drizzle.config.ts` | `@getknext/db/migrate` → `defineDrizzleConfig()` |
...
Inherited from `@getknext/lib` (ADR-0019): pool idle timeout **< 60s** gateway idle
