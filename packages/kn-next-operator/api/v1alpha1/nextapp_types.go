...
type SecuritySpec struct {
	...
	// shape only — Next's own optimized-image variant cache under
	// `.next/standalone/.next/cache/images`) are provisioned as explicit
	// emptyDir mounts so the app keeps working with a read-only root; the
	// vinext single-executable shape needs neither, so it renders with no
	// extra mounts. The V8/Node compile-cache directory baked into the
	// standalone image needs no mount: Node's compile cache is fail-open on
	// an unwritable directory (falls back to the baked, read-only entries).
	//
...
type ScalingSpec struct {
	...
	// NOW (in each window's timezone) and sets the ksvc
	// `autoscaling.knative.dev/min-scale` to max(spec.scaling.minScale, the active
	// window's replicas). The Knative KPA reads that as its scale FLOOR and still
	// scales ABOVE it reactively. The operator RequeueAfter's the next window
	// boundary so the floor flips at start/end. Because the operator is the sole
	// writer, the floor never reverts/thrashes (no CronJob, no KEDA — KEDA cannot
	// scale a Knative Service; an external CronJob writer was reverted every
	// reconcile). Outside every window the floor is spec.scaling.minScale (default
	...
	// a DB-touching request can still pay the database wake (measured on the
	// file-manager spike, #766: 290 ms with the compute awake vs ~2.3 s
	// waking it — the wake figure is recorded in
	// docs/benchmarks/fm-same-source-oke-ab-2026-08-18.md). The two windows are
	...
	// AppDatabase (apps.scale-zero-pg.dev) — `spec.tier: warm` for 24/7, or
	// `spec.warmSchedule` for declared hours (#777) — knext's operator never
	// reads or writes AppDatabase (ADR-0001 boundary).
	//
...
type WarmWindow struct {
	// Start is the cron expression at which the warm floor begins. Standard
	// 5-field cron (e.g. "0 8 * * 1-5" = 08:00 on weekdays); syntax-validated at
	// admission.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinLength=1
	Start string `json:"start"`

	// End is the cron expression at which the warm floor ends (e.g. "0 20 * * 1-5"
	// = 20:00 on weekdays). Between Start and End the floor of `replicas` pods is
	// held. Standard 5-field cron; syntax-validated at admission.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinLength=1
	End string `json:"end"`

	// Replicas is the warm-pod FLOOR held during the window (the min-scale the
	// operator stamps on the ksvc while now is inside this window). Must be >= 1 —
	// a window that floors at 0 warms nothing (use no window at all for
	// scale-to-zero). The Knative KPA still scales ABOVE this floor on real
	// traffic.
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:Minimum=1
	...
	// schedules are evaluated in when the operator tests window membership.
