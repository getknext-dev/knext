...
// the supervisor itself emits, so each interval is attributed from inside the
// process rather than inferred from outside it:
//
...
//   child-listening → supervisor-ready the deferred init, off the critical path
//
// Two reference arms put those numbers in context:
//   FLOOR  = `node -e ""`             — the bare process cost nothing can remove
//   DIRECT = `node <server>`          — the child booted with no supervisor at all
//
// The child-listening mark is detected by the supervisor's own TCP probe, so its
// resolution is the probe interval; the harness tightens it to 10ms
// (KNEXT_CHILD_READY_PROBE_MS) for measurement. Production keeps the 250ms default.
...
if (dep.status !== 0 || !existsSync(join(deploy, 'dist/adapters/node-server.js'))) {
...
// 3. The child: a real standalone build if given one, else a fast fixture.
...
// A real standalone server.js resolves its deps relative to itself, so run it
...
async function traceOnce(port, deadlineMs = 60_000) {
  const marks = [];
  const child = spawn(process.execPath, ['-e', "import('@getknext/core/internal/node-server')"], {
    cwd: childCwd,
    env: {
      ...process.env,
      PORT: String(port),
...
/** Wall-clock time for a plain process to reach a listening socket / exit. */
...
for (let i = 0; i < N; i++) {
    ...
    for (const m of marks) {
        // First occurrence wins; a phase is marked once per boot.
...
console.log(
    `\nreference  FLOOR (node -e ""):        median ${fmt(median(floors))}ms  [${range(floors)}]`,
);
console.log(
