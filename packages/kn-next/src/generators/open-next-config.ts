 ...
 */
function getQueueAdapter(config: KnativeNextConfig, enableKafkaQueue?: boolean): string {
  // First check if config.queue is explicitly set
  if (config.queue) {
    return config.queue.provider === 'kafka' ? ADAPTER_MAP.queue.kafka : ADAPTER_MAP.queue.direct;
  }
  // Fall back to enableKafkaQueue flag or default to Kafka for Knative
  return enableKafkaQueue !== false ? ADAPTER_MAP.queue.kafka : ADAPTER_MAP.queue.direct;
}
 ...
 */
export function generateOpenNextConfig(options: GenerateOpenNextConfigOptions): string {
  ...
  const storageAdapter = useRedisForCache
    ...
    : (ADAPTER_MAP.storage[config.storage.provider as keyof typeof ADAPTER_MAP.storage] ??
  ...
  const queueProvider = queueAdapter === ADAPTER_MAP.queue.kafka ? 'kafka' : 'direct';
 ...
 */
type EnvVarGenerator<T> = (config: T, appName: string) => Record<string, string>;
 ...
 */
export function getRequiredEnvVars(config: KnativeNextConfig): Record<string, string> {
  const storageGenerator = storageEnvVarGenerators[config.storage.provider];
  const cacheGenerator = config.cache ? cacheEnvVarGenerators[config.cache.provider] : null;
