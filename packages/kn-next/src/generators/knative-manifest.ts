...
const log = createLogger({ module: "manifest" });
...
export function generateKnativeManifest(
    ...
    log.info({ path: outputPath }, "Generated Knative service manifest");
    log.info({ path: imageCachePath }, "Generated image cache manifest");
    return outputPath;
}
...
export function generateKafkaEventingManifest(options: {
    ...
    log.info({ path: outputPath }, "Generated Kafka eventing manifest");
    return outputPath;
}
