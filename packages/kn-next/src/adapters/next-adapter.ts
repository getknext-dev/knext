...
const adapter: NextAdapter = {
  ...
  modifyConfig(config, { phase }) {
    ...
    console.log('[knext-adapter] modifyConfig fired for phase-production-build');
  ...
  async onBuildComplete(ctx) {
    ...
    if (ctxAny.routes) {
      ...
      routingCounts.rewritesBeforeFiles = len(routes.rewrites?.beforeFiles);
...
async function uploadBuildArtifacts({
  ...
  try {
    ...
    console.log(`[knext-adapter] upload skipped: could not load storage client — ${String(err)}`);
    return;
  }
  ...
  for (const { filePath, key } of artifacts) {
    ...
    try {
      ...
      console.log(`[knext-adapter] upload warning: failed to upload "${key}" — ${String(err)}`);
      skipped++;
    }
  }
