 ...
 *    @getknext/lib/clients — so draining never errors in the no-RO-pool case;
 ...
 * runtime package so @getknext/lib's pool stays free of any dependency on the runtime
 * or ./shutdown — no circular dependency.
 *
 * ## Why the clients module is imported LAZILY (#441)
 *
 * `@getknext/lib/clients` statically pulls `@cerbos/grpc`, `minio` and `pg` — by far
...
/** The slice of `@getknext/lib/clients` the drain needs. */
...
const loadDbClients: DbClientsLoader = () => import("@getknext/lib/clients");

export interface DrainDbPoolsOptions {
    /** Injected for tests; defaults to the lazy `@getknext/lib/clients` import. */
