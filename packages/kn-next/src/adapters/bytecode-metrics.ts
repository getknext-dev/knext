...
export function initBytecodeCacheMetrics(): void {
    ...
    if (!cacheDir) {
        console.log('[kn-next] Bytecode cache metrics: NODE_COMPILE_CACHE not set, skipping');
        cacheWarmStart.set({ app: APP_NAME, build_id: BUILD_ID }, 0);
        return;
    }

    console.log(`[kn-next] Bytecode cache metrics: scanning ${cacheDir}`);
...
export function recordServerReady(): void {
    ...
    console.log(`[kn-next] Server ready in ${(uptimeSeconds * 1000).toFixed(0)}ms (${cacheStatus} cache)`);
}
