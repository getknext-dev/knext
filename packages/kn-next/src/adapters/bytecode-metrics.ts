...
const APP_NAME =
    process.env.KN_APP_NAME || process.env.npm_package_name || "unknown";
...
function watchCacheWrites(cacheDir: string): void {
    ...
    const checkInterval = setInterval(() => {
        try {
            ...
            if (newFiles > 0) {
                cacheWriteCount.inc(
                    { app: APP_NAME, build_id: BUILD_ID },
                    newFiles,
                );
                lastCount = fileCount;
...
export function initBytecodeCacheMetrics(): void {
    ...
    if (!cacheDir) {
        console.info(
            "[kn-next] Bytecode cache metrics: NODE_COMPILE_CACHE not set, skipping",
        );
