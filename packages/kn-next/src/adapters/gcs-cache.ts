...
function getBucket() {
  if (!_bucket) {
    ...
    if (!bucketName) {
      throw new Error('[GCS Cache] GCS_BUCKET_NAME environment variable is required');
    }
...
const incrementalCache: IncrementalCache = {
  async get<CacheType extends CacheEntryType = 'cache'>(
    ...
    try {
      const file = getBucket().file(buildGcsKey(key, cacheType ?? 'cache'));
      ...
      return {
        value: cacheData,
        lastModified: metadata.updated ? new Date(metadata.updated).getTime() : Date.now(),
      };
    } catch (error) {
  ...
  async delete(key: string): Promise<void> {
    ...
    try {
      ...
      console.error('[GCS Cache] Error deleting cache entry:', key, error);
    }
  },
