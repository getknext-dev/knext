...
function stage(): string {
    const dir = tempDir("knext-sharp-shim-");
...
describe("the shim the compile injects is VERBATIM and self-contained", () => {
    ...
    // #949 C-1b: the injection-filter ↔ template-sharp-pin coupling moved to
    // tests/template-sharp-pin.test.ts, which scans EVERY templates/**
    // package.json(.hbs) via the shared walk — the one-template version here
    // left the zone scaffold unguarded (review round F1).
});
...
describe("sharp addon path resolution", () => {
    ...
    it("falls back to a flat sharp.node beside the executable", () => {
        const dir = tempDir("knext-sharp-flat-");
...
function stageVerifiable(): {
    ...
    const dir = tempDir("knext-sharp-verify-");
...
describe("sharp addon integrity verification", () => {
    ...
    it("warns and loads when there is no manifest â never bricks an older image", () => {
        ...
        const dir = tempDir("knext-sharp-legacy-");
