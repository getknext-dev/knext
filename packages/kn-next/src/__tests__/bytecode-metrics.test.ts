...
describe("Bytecode Cache Metrics", () => {
    ...
    it("should detect cold start when cache directory is empty", async () => {
        ...
        const warmStartMetric = metrics.find(
            (m) => m.name === "kn_next_bytecode_cache_warm_start",
        );
    ...
    it("should detect warm start when cache files exist", async () => {
        ...
        const filesMetric = metrics.find(
            (m) => m.name === "kn_next_bytecode_cache_files_total",
        );
    ...
    it("should measure cache size in bytes", async () => {
        ...
        const metrics = await metricsRegistry.getMetricsAsJSON();
        const sizeMetric = metrics.find(
            (m) => m.name === "kn_next_bytecode_cache_size_bytes",
        );

        expect((sizeMetric as any)?.values?.[0]?.value).toBeGreaterThanOrEqual(
            1024,
        );
    });
    ...
    it("should record startup duration when recordServerReady is called", async () => {
        ...
        const { recordServerReady, metricsRegistry } = await import(
            "../adapters/bytecode-metrics"
        );
        ...
        const durationMetric = metrics.find(
            (m) => m.name === "kn_next_startup_duration_seconds",
        );
    ...
    it("should expose metrics in Prometheus text format", async () => {
        ...
        const { initBytecodeCacheMetrics, recordServerReady, metricsRegistry } =
            await import("../adapters/bytecode-metrics");
