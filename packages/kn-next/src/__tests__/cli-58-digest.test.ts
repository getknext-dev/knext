...
describe('validateTaggedRef', () => {
  it('accepts a normal registry/name:tag ref', () => {
    expect(() => validateTaggedRef('registry.example.com/my-app:20240101')).not.toThrow();
  });

  it('accepts a digest-pinned ref', () => {
    expect(() =>
      validateTaggedRef(`registry.example.com/my-app:v1@sha256:${FAKE_DIGEST.slice(7)}`),
    ).not.toThrow();
  });

  it('rejects a ref containing semicolon (shell command separator)', () => {
    expect(() => validateTaggedRef('registry.example.com/my-app:tag;touch x')).toThrow(/invalid/i);
  });

  it('rejects a ref containing backtick (shell command substitution)', () => {
    expect(() => validateTaggedRef('registry.example.com/my-app:tag`id`')).toThrow(/invalid/i);
  });
  ...
  it('rejects a ref containing space', () => {
    expect(() => validateTaggedRef('registry.example.com/my-app:tag extra')).toThrow(/invalid/i);
  });

  it('rejects a ref containing newline', () => {
    expect(() => validateTaggedRef('registry.example.com/my-app:tag\necho hi')).toThrow(/invalid/i);
  });
});
...
describe('resolveDigest — docker-inspect fallback (FALLBACK path)', () => {
  ...
  it('exec spy receives taggedRef as a standalone array element (injection-safety proof)', async () => {
    ...
    await expect(resolveDigest(metaCharRef, execSpy)).rejects.toThrow(/invalid/i);
