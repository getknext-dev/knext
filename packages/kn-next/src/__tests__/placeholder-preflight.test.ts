...
describe('findPlaceholders — generic scan over the config object', () => {
  it("finds the scaffold's live placeholder (registry)", () => {
    ...
    expect(findings).toEqual([{ path: 'registry', value: 'ghcr.io/<your-user>' }]);
  });
  ...
  it('finds a placeholder in a nested field (storage.bucket)', () => {
    ...
    expect(findings).toEqual([{ path: 'storage.bucket', value: '<your-assets-bucket>' }]);
  });
  ...
  it('ABSENT storage is not a placeholder error (ADR-0047 valid mode)', () => {
    expect(findPlaceholders({ name: 'my-app', registry: 'ghcr.io/real' })).toEqual([]);
  });
  ...
  it('dodge 2: a placeholder inside an array element is still found', () => {
    ...
    expect(findPlaceholders(cfg)).toEqual([{ path: 'domains[1]', value: '<your-domain>' }]);
  });

  it('env values are scanned like everything else', () => {
    const cfg = {
      ...clean,
      env: { API_URL: 'https://api.example.com', API_KEY: '<set-me>' },
    };
    expect(findPlaceholders(cfg)).toEqual([{ path: 'env.API_KEY', value: '<set-me>' }]);
  });
...
describe('PlaceholderConfigError — the friendly write-and-exit path', () => {
  it('is a UsageError-family error, so every entry renders it as a message', () => {
    const err = new PlaceholderConfigError([{ path: 'registry', value: 'ghcr.io/<your-user>' }]);
  ...
  it('a non-placeholder error is NOT claimed by the handler (mutation half)', () => {
    const chunks: string[] = [];
    expect(handleUsageError(new Error('boom'), (t) => chunks.push(t))).toBe(false);
