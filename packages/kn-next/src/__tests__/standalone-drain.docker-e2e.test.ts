...
afterAll(() => {
    ...
    // daemon (observed locally). Each `run()` call above already carries its
    // own 60s spawnSync timeout; this just gives the HOOK itself room to run
    // them all without bun killing it first.
}, 300_000);
