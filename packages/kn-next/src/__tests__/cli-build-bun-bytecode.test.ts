 ...
 */
// 60s, not 30s: the `composes with the bun-exports heal` case runs TWO real
// bun builds, so its floor is ~8.7s before any contention — and under
// full-suite parallelism it still blew a 30s budget while passing comfortably
// when the file ran alone. Sized for the loaded case, because that is the one
// CI actually runs.
const BYTECODE_BUILD_TIMEOUT_MS = 60_000;

describe(
    "precompileBunBytecode (module)",
    {
            ...
            () => {
                ...
                expect(
                    existsSync(
                        join(
                            standaloneDir,
                            ".next/static/chunks/client.js.jsc",
                        ),
                    ),
                ).toBe(false);
            },
...
describe(
    "kn-next build — bytecode pass gating (runtime knob)",
    {
        timeout: BYTECODE_BUILD_TIMEOUT_MS,
    },
    () => {
