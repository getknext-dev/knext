 ...
 * `knext deploy` applies the NextApp CR with an explicit `--validate=strict`
...
export function kubectlValidationCheck(ctx: CheckContext): CheckResult[] {
    ...
    return [
        mk(
            ...
            "kubectl strict validation",
            "fail",
            `client ${parsed.display} is older than v${MIN_STRICT_VALIDATION_KUBECTL.major}.${MIN_STRICT_VALIDATION_KUBECTL.minor} — before v1.25 --validate is a BOOLEAN, so \`knext deploy\` fails on this client at flag parsing, before it contacts the apiserver. Upgrade kubectl`,
